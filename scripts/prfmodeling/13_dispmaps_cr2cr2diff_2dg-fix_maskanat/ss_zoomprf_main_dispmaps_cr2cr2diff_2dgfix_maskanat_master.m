%% -----------------------------------------------------------------------------
% 13 - zoomprf main - display cross-validated R2 and R2 difference map for 
%      2dg-fix model with masked anatomy
% ------------------------------------------------------------------------------
%
%
% ------------------------------------------------------------------------------
...
Para.MapType             = {'cR^2' 'diff-cR^2'};
Para.PathColors          = {[1 1 1] [1 1 1]};
Para.Mesh                = 'inflated';
Para.EccenRange          = [0 Inf];
Para.NR2ThreshGen        = 0;
%%% Note that "Gen" refers to general.
Para.CR2Thresh           = [0 0.8];
Para.DiffCR2Thresh       = [0 0.08];

Para.Threshold           = {...
    [Para.NR2ThreshGen Para.CR2Thresh      Para.EccenRange Para.Transparency]; ...
    [Para.NR2ThreshGen Para.DiffCR2Thresh  Para.EccenRange Para.Transparency]};
...
try
    ...
    %% -------------------------------------------------------------------------
    % (1) Display cross-validated R2 map and R2 difference map for 2dg-fix model 
    %     with masked anatomy
    % --------------------------------------------------------------------------
