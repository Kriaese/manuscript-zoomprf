...
# 4) NearestNeighbor interpolation instead of linear interpolation
# 5) Radius for CC similarity metric changed to 5 (from 4) - Radius^Dimension neighbors should be used
# 6) Allows for rigid-only registration using '-t rigid' (for intrasession registration) 
...
Optional arguments:
     ...
     -g:  Gradient step size (default 0.1): smaller in magnitude results in more
          cautious steps.
     ...
     -n:  N4BiasFieldCorrection of moving image: 0 == off, 1 == on (default 0).

     -p:  Commands to prepend to job scripts (e.g., change into appropriate directory, set
          paths, etc)

     -r:  Do rigid-body registration of inputs before creating template (default 1):
     ...
     -t:  Type of transformation model used for registration (default = SyN):  Options are
	    ...
	    Rigid = Rigid only (for intrasession registration)
...
(Intrasession) example (rigid only):

`basename $0` -d 3 -c 0 -e 1 -i 1 -k 1 -f 8x4x2x1 -s 4x2x1x0 -q 250x250x100x50 -a 0 -b 0 -n 0 -t Rigid -r 1 -o Intrasession_ MPRAGE_Vol*_Tukey-02_adaptComb-20x20x5.nii.gz |& tee ANTs.log

Intersession example (rigid and non-linear):

`basename $0` -d 3 -c 0 -e 1 -i 1 -k 1 -f 8x4x2x1 -s 4x2x1x0 -q 100x75x50x20 -a 0 -b 0 -n 0 -t SyN -m CC -r 1 -o Intersession_ MPRAGE_Vol*_Tukey-02_adaptComb-20x20x5.nii.gz |& tee ANTs.log

Multiple-iteration intersession with initial template (for better results?):

`basename $0` -d 3 -c 0 -e 1 -i 4 -k 1 -f 8x4x2x1 -s 4x2x1x0 -q 250x250x100x50 -a 2 -b 0 -n 0 -t SyN -m CC -r 1 -o Init_Intersession_ MPRAGE_Vol*_Tukey-02_adaptComb-20x20x5.nii.gz |& tee ANTs.log
`basename $0` -d 3 -c 0 -e 1 -i 4 -k 1 -f 8x4x2x1 -s 4x2x1x0 -q 250x250x100x50 -a 0 -b 0 -n 0 -t SyN -m CC -r 0 -z Init_Intersession_template0.nii.gz -o Intersession_ MPRAGE_Vol*_Tukey-02_adaptComb-20x20x5.nii.gz |& tee ANTs.log

Multimodal example:

`basename $0` -d 3 -i 3 -k 2 -f 4x2x1 -s 2x1x0vox -q 30x20x4 -t SyN -z t1.nii.gz -z t2.nii.gz -m CC -c 0 -o MY templateInput.csv
...
--------------------------------------------------------------------------------------
Script originally by Nick Tustison
--------------------------------------------------------------------------------------
Script modified by Falk Luesebrink (falk.luesebrink [at] ovgu.de) based on antsMultivariateTemplateConstruction2.sh
--------------------------------------------------------------------------------------
...
# This keeps the template shape stable over multiple iterations of template building
    ...
    if [[ $whichtemplate -eq 0 ]];
      then
        ...
        echo "   ${WARP} -d ${dim} -e vector -n NearestNeighbor -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [${templatename}0GenericAffine.mat,1] -r ${template} --verbose 1"
        echo "--------------------------------------------------------------------------------------"
        ...
        if [[ $NWARPS -ne 0 ]];
          then
            ${WARP} -d ${dim} -e vector -n NearestNeighbor -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [${templatename}0GenericAffine.mat,1] -r ${template} --verbose 1
    ...
    echo "   ${WARP} -d ${dim} --float $USEFLOAT -n NearestNeighbor --verbose 1 -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}"
    echo "--------------------------------------------------------------------------------------"

    if [ -f "${templatename}0warp.nii.gz" ];
      then
        ${WARP} -d ${dim} --float $USEFLOAT -n NearestNeighbor --verbose 1 -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}
      else
        ${WARP} -d ${dim} --float $USEFLOAT -n NearestNeighbor --verbose 1 -i ${template} -o ${template} -t [${templatename}0GenericAffine.mat,1] -r ${template}
      fi
  ...
  then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
      do
        ...
        #basecall="${ANTS} -d ${DIM} --float $USEFLOAT --verbose 1 -n NearestNeighbor -u 1 -w [0.01,0.99] -z 1 -r [${TEMPLATES[0]},${IMAGESETARRAY[$i]},2]"
		basecall="${ANTS} -d ${DIM} --float $USEFLOAT --verbose 1 -n NearestNeighbor -u 1 -w [0.01,0.99] -z 1 -r [${TEMPLATES[0]},${IMAGESETARRAY[$i]},0]"
        ...
        for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
          do
            k=0
            let k=$i+$j
            IMAGEMETRICSET="$IMAGEMETRICSET -m Mattes[${TEMPLATES[$j]},${IMAGESETARRAY[$k]},${MODALITYWEIGHTS[$j]},32,Regular,0.25]"
          done

		stage1="-t Rigid[0.1] ${IMAGEMETRICSET} -c [1000x750x500x250,1e-7,10] -f 6x4x2x1 -s 3x2x1x0 -o ${outdir}/rigid${i}_"
        exe="${basecall} ${stage1}"
        ...
        for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
          do
            ...
            exe2="$exe2 ${WARP} -d $DIM --float $USEFLOAT -n NearestNeighbor --verbose 1 -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]}\n"
            pexe2="$exe2 ${WARP} -d $DIM --float $USEFLOAT -n NearestNeighbor --verbose 1 -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]} >> ${outdir}/job_${count}_metriclog.txt\n"
          done
...
while [[ $i -lt ${ITERATIONLIMIT} ]];
  do
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
      do
	    basecall="${ANTS} -d ${DIM} --float $USEFLOAT --verbose 1 -n NearestNeighbor -u 1 -w [0.01,0.99] -z 1"
          ...
          do
            ...
            if [[ "${METRICTYPE[$k]}" == "DEMONS" ]];
              ...
              then
                # Mapping Parameters
                METRIC=Mattes[
				METRICPARAMS="${MODALITYWEIGHTS[$k]},32,None,1]"
            ...
            if [[ $N4CORRECT -eq 1 ]];
              then
                ...
                IMAGEMETRICLINEARSET="$IMAGEMETRICLINEARSET -m Mattes[${TEMPLATES[$k]},${REPAIRED},${MODALITYWEIGHTS[$k]},32,Regular,0.25]"

                warpexe=" $warpexe ${WARP} -d ${DIM} -n NearestNeighbor --float $USEFLOAT --verbose 1 -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} -n NearestNeighbor --float $USEFLOAT --verbose 1 -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              else
                IMAGEMETRICSET="$IMAGEMETRICSET -m ${METRIC}${TEMPLATES[$k]},${IMAGESETARRAY[$l]},${METRICPARAMS}"
                IMAGEMETRICLINEARSET="$IMAGEMETRICLINEARSET -m Mattes[${TEMPLATES[$k]},${IMAGESETARRAY[$l]},${MODALITYWEIGHTS[$k]},32,None,1]"

                warpexe=" $warpexe ${WARP} -d ${DIM} -n NearestNeighbor --float $USEFLOAT --verbose 1 -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} -n NearestNeighbor --float $USEFLOAT --verbose 1 -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              fi
        ...
        stage0="-r [${TEMPLATES[0]},${IMAGESETARRAY[$j]},0]"
        stage1="-t Rigid[0.1] ${IMAGEMETRICLINEARSET} -c [1000x750x500x250,1e-8,10] -f 6x4x2x1 -s 4x2x1x0"
        stage2="-t ${TRANSFORMATION} ${IMAGEMETRICSET} -c [${MAXITERATIONS},1e-8,10] -f ${SHRINKFACTORS} -s ${SMOOTHINGFACTORS} -o ${outdir}/${OUTWARPFN}"

        stageId="-t Rigid[0.1] ${IMAGEMETRICLINEARSET} -c [0,1e-4,10] -f 1 -s 0"
        exebase=$exe
        pexebase=$pexe
		if [[ $TRANSFORMATIONTYPE == "Rigid"* ]];
		  then
  		        exe="$exe ${basecall} ${stage0} ${stage2}\n"
			pexe="$pexe ${basecall} ${stage0} ${stage2} >> ${outdir}/job_${count}_metriclog.txt\n"
		  else
			if [[ $DOLINEAR -ne 0 ]];
			  then
				exe="$exe ${basecall} ${stage0} ${stage1} ${stage2}\n"
				pexe="$pexe ${basecall} ${stage0} ${stage1} ${stage2} >> ${outdir}/job_${count}_metriclog.txt\n"
			  else
				exe="$exe ${basecall} ${stageId} ${stage2}\n"
				pexe="$pexe ${basecall} ${stageId} ${stage2} >> ${outdir}/job_${count}_metriclog.txt\n"
			  fi
			if [[ $NOWARP -eq 0 ]];
			  then
